...
def IntepreteASOS(csvfile,STATION,ID):
    ...
    with open(csvfile) as filt_csv:
        ...
        ##asos.loc[:,'obs_time_standard']=asos['observation_time'].apply(UTC2STANDARD)

        #print asos.loc[:,'obs_time_local'],asos.loc[:,'obs_time_standard']
...
zuluHour = int(zuluTimeNow.strftime("%H"))
print 'zuluHour:',zuluHour
#Whatever the time on the local maching, the ZULU time for getting the recent records
#should be in the end of ZULU 18 hour (e.g. 18:51:00Z)
#Because I am running twice at 13:00 hour and 13:25 at local time so the 19 hour should be also
##if zuluHour < 18 or zuluHour > 19 :
##    print 'quit the process because the hour is not between 18~20 ZULU',
##    logging.info("quit the process because the hour is not between 18~20 ZULU")
##    exit()
##else:
##    print "Processing the ASOS Fire Weather for today: ",today.strftime("%Y%m%d")
