#------------------------------------------------------------------------------------
# Name:        ASOS2WIMSTEST_MesoWest.py
...
# Update 02/23/2018 to try the differences for windspeed in the reporting time and 5 minutes
...
#ignore the invalid value warning
...
def StateOfWeather(X, row):
    ...
    elif not wxstring.find('Fog') == -1 and row['RelativeHumidity'] >= 95: #other situation will be 1!
        X['State of Weather'] = 4
    # other situation will be 1!
    elif not wxstring.find('Haze') == -1 or not wxstring.find('Blowing Dust') == -1:
        X['State of Weather'] = 1            
    ...
    else:  # only to see if there are rain or more several situations
        X['State of Weather'] = 3
...
##    X['Ob Time']=  row['obs_time_local'].strftime("%H%M")
    ...
    #Following the updating on 07192017 for resoving the wims issue with 13:00 observation timestamp
...
# Based on the discussion with Mike and Curt etc on 02/26/2017
# The windspeed will used only 3 records that reflacts the 50,55 and the end of hour which disregard the flag minutes (seems no need to use end hour report anymore)
def Last15Min(dfgrouped):
    #need to remove the 0 record into calculation
    #last10MinutesWind = dfgrouped[-4:].median()
    listValue = dfgrouped[-4:].tolist()
    NoNullListValue = [x for x in listValue if x != 0]
    if len(NoNullListValue) == 0:
        last10MinutesWind = 0
    else:
        last10MinutesWind = np.median(NoNullListValue)
    #print last10MinutesWind
    # for g in dfgrouped[-4:0]:
    #     print g
    #print type(dfgrouped) #dfgrouped[-10:]
    #print type(dfgrouped[-10:])
    # print len(dfgrouped)
    # if type(dfgrouped) == 'pandas.core.frame.DataFrame':
    #     print dfgrouped
    # return 0
...
def IntepreteASOSWest(asos, STATION, ID):
    ...
    # df_shift = asos
    # df_shift.loc[:, 'date_time_shift'] = df_shift.loc[:,('date_time')].shift(1)
    # #print df_t.loc[:, ['date_time', 'date_time_shift', 'wind_speed_set_1']]
    # df_shift.drop(0, inplace=True)
    # print df_t.loc[:, ['date_time', 'date_time_shift', 'wind_speed_set_1']]
    #Seems working, need to trim the first row 
    asos.loc[:, 'date_time_shift'] = asos.loc[:, ('date_time')].shift(1)
    asos.drop(0, inplace=True)
    #need to replace the ('date_time') with 'date_time_shift
    asos.loc[:, 'date_time'] = asos.loc[:, 'date_time_shift']
    #print asos.loc[:, ('date_time', 'date_time_shift', 'wind_speed_set_1')]
#asos.loc[:, 'METAR'] = asos.loc[:"metar_set_1"][0][0:3]
    ...
    # Leave potential improvements for future
    # using a modulo for division of current minutes to 5 to get the 5 min frequency data
    # asos.loc[:, 'minutes'] = asos.loc[:, ('date_time')].str[14:16].apply(lambda t: int(t))
    # #print asos.loc[:, 'minutes']
    # asos_ws = asos[asos.loc[:, 'minutes'] % 5 == 0 ]
    # asos_ws = asos_ws[['wind_speed_set_1','wind_direction_set_1','date_time','minutes','Date_Hour']]
    # #try to figure out more tricks on getting the last two records of 5 min frequency data and new hour data
    # #Don't think it is necessary for this implementation which will cause a lot confusion, using the last the last three records instead!
    # asos_ws.loc[:, 'processed_wind_speed'] = asos_ws.loc[:, 'wind_speed_set_1'].groupby(asos_ws['Date_Hour']).transform(Last10Min)
    # print asos_ws
    #Process the wind speed 
    asos.loc[:, 'processed_wind_speed'] = asos.loc[:, 'wind_speed_set_1'].groupby(asos['Date_Hour']).transform(Last15Min)
    asos.loc[:, 'processed_wind_direction'] = asos.loc[:, 'wind_direction_set_1'].groupby(asos['Date_Hour']).transform(Last15Min)

    #print asos.loc[:, 'processed_wind_speed']
    ...
    asos[['air_temp_set_1', 'relative_humidity_set_1',
            'wind_speed_set_1', 'wind_direction_set_1', 'precip_accum_one_hour_set_1',
            ]] = asos[['air_temp_set_1', 'relative_humidity_set_1',
                        'wind_speed_set_1', 'wind_direction_set_1', 'precip_accum_one_hour_set_1', ]].astype(float)
    #print(asos.head(10))
    ...
    asos.loc[:, 'WindSpeed'] = asos.loc[:, 'processed_wind_speed']
    asos.loc[:, 'WindDirection'] = asos.loc[:, 'processed_wind_direction']
    ...
    ASOS = asos[['date_time', 'Precipitation', 'AirTemperature', 'RelativeHumidity',
                    'MaxAirTemperature', 'MinAirTemperature', 'WindSpeed', 'WindDirection', 'Date_Hour', 'WeatherCode', 'Metar']]
    ...
    X13 = {'W13': 'W13', 'Station Number': '000000', 'Ob Date': 'YYYYMMDD', 'Ob Time': 0,
            ...
            'PrecipAmt': 0, 'WetFlag': 'N', 'Herb': 20, 'Shrub': 15,
            'MoistType': 2, 'MeasType': 1, 'SeasonCode': 3, 'SolarRad': 0,
            # Updated 02/03/2016 for the new three parameters
            'GustDir': 0, 'GustSpeed': 0, 'SnowFlag': 'N'
    ...
    ##X13['Station Number'] = ID
    fileWF13fn =  STATION + '.fw13'
    fileWF9 = os.path.join(WorkSpace, fileWF13fn)
...
#Updated on 03/01/2018 using a CSV (GreenessCode as a input for all the stations)
csv_pd = pandas.DataFrame.from_csv(GreenessCodeCSV)
#print csv_pd
Stations = dict(zip(csv_pd.loc[:, 'Station Identifier'], csv_pd.loc[:, 'WIMS ID']))
...
fileWF9 = os.path.join(WorkSpace, "tx-mesowest-asos.fw9")
...
#Try to compute the last records from the last 2 record (high frequency) near the end hour and the first record of next hour
START_UTC = "1900"
END_UTC = "1905"
...
varList = ['wind_direction', 'weather_condition', 'wind_direction', 'precip_accum_one_hour', 'peak_wind_direction', 'peak_wind_speed','wind_speed',
           'relative_humidity', 'precip_accum_three_hour', 'dew_point_temperature', 'air_temp', 'metar', 'precip_accum_24_hour']
...
for stationobs in alldata['STATION']:
    ...
    if 'weather_condition_set_1d' not in variablelist:
        df.loc[:, 'weather_condition_set_1d'] = ''
    #print df
