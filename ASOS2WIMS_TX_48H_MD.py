...
def windspeed(series):
    ...
    return series['wind_speed_kt'] * 0.9 * 1.15077944802  * math.log(6/0.0984)/math.log(10/0.0984)
    #updated on 10-13-2015: Mike requet to reduce wind 10% down and Larry's correction on the wind speed unit
    #return gust * math.log(6/0.0984)/math.log(10/0.0984)*0.90
...
# Input : csv file for the precious 48 hours;station name;diction for precipitation from IEM
...
def IntepreteASOS(csvfile,STATION,ID):
    ...
    with open(csvfile) as filt_csv:
        ...
        #check the 2 days observation for filling the temperature and dewpoint if missing value(h
        #need to think aobut the first hour is missing!
        #Missing value checking function here
        ...
        #Updated 10-15-2015, Can not fill nodata for temp with 0 (32 F)
        asos['temp_c'].fillna(-999, inplace=True)
...
##            asos.loc[hour,'PRCP3HOUR']=asos.loc[hour,'precip_in'] + asos.loc[hour+1,'precip_in'] + asos.loc[hour+2,'precip_in']
        ...
        #10-15-2015 suggested by Mike: set all season codes (SC) to a 3, the grass greenness (GGF) to 20 and the shrub greenness (SGF) to a 15.
...
def Report(asos,X,row):
    ...
    #Default for 15 suggested by Mike
...
#Upload the file to the WebDAV server through a mapped network drive("w:")
##url = "https://famtest.nwcg.gov/fam-web/wims_dav/incoming"
#user = "mdunivan"
#passwd = "T43Gr8T3X@S!"
#shutil.copyfile(fileWF9,"w:\\tx-asos.fw9")
