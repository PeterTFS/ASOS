...
def checkValidRecords(df):
    ...
    #What happens if the current hour df.head(1) (near 13 observation observation is missing)? Need to notify!
    TIMESTR = df.iloc[0]['observation_time']
    print TIMESTR
    FstRcdT = RoundHour ( UTC2LOCAL(TIMESTR) )
    if FstRcdT != 13:
        MSG = "The 13:00 observation for Station: %s is missing "% (df.iloc[0]['station_id'])
        print MSG
#        logging.info(MSG)
#        sendEmail(MSG)
#        sendEmail("")
    print "timeBegin:",timeBegin,'FstRcdT:',FstRcdT
    for index,row in df.iterrows():
        ...
        if(tdelta.days>= 1):
            ...
            logging.info("There is an unvalid record for Station: %s",row['station_id'])
    print 'recordsToRemove:',recordsToRemove
...
def IntepreteASOS(csvfile,STATION,ID):
    ...
    with open(csvfile) as filt_csv:
        ...
        #asos['temp_c'].fillna(NaN, inplace=True)
        #asos['RH'].fillna(NaN, inplace=True)
        #Relative Humidity is based on temperature and dewpoint(is should not be derived from here because no data issue
...
##            asos.loc[hour,'PRCP3HOUR']=asos.loc[hour,'precip_in'] + asos.loc[hour+1,'precip_in'] + asos.loc[hour+2,'precip_in']
        ...
        #pass the station ID
        X['Station Number'] = ID
        #Create a file with fixed name ('tx-asos_KDHT-201510311300.fw9') suggested by Larry
        with open(fileWF9,'a') as F:
            ...
            #print df
            print 'validRecordsLength:',validRecordsLength
...
fileWF9 = os.path.join(workspace, "tx-asos-debug.fw9")
...
#############################################################
csvFN = "C:\\DEV\ASOS\CSV\\KINK-201511101300.csv"
