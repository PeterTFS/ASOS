#---------------------------------------------------------------------------------------------------
# Name:        ASOS2WIMS_TX_HISTORY_FW13_UpdatedWind.py
# Purpose:     Process downloaded historical ASOS hourly record to FW13 forwat (only "O" record)
# Author:      pyang
# Created:     11/4/2015
# Updated:     01092016 for windspeed adjusting
# Updated:     03/06/2017 for windspeed (nodata --> 3 ) and wind direction (nodata --> 0)
#---------------------------------------------------------------------------------------------------
import time
import os
import sys
...
#Updated for the transformation from no dato to 3
#---------------------------------------------------------------------------------------------------------------------------------------------------------
...
def FormatFW9( X ):
    ...
    for f,p in Fields:
        ...
        WindParaList = ['Temp','Tmax','Tmin','Moisture','RHmax','RHmin','10hr Fuel']#updated on 04/14/2016 Mike noticed missing wind speed and direction records rejected by WIMS
    ...
    for f,p in Fields:
        ...
        if f=='PrecipAmt':
            if val == 0:
                ...
                val*=1000
        ##Updated on 03/06/2017 for replacing the nodata vaule {Windspeed to 3,
...
##                    print X13
                    ...
                    ##Only write the 'o' record to the FW13 file
##                    if X13['Type']=='O':
##                        print X13
##                        F13.write( FormatFW13( X13 ) +'\n' )
...
##03/03/2017
#Mike need anther 3 stations Huntsville, Victoria and Midland for comparing it to RAWS
# Huntsville: UTS
# Midland :  MDD
# Victoria : VCT
Stations = {"KUTS": 414103,
            "KMAF": 419205,
            "KVCT": 418203
            }
...
starts = datetime(2016, 7, 1)
endts = datetime(2016, 7, 19)

starts = datetime(2000, 7, 1)
endts = datetime(2017, 3, 3)

starts = datetime(2017, 3, 3)
endts = datetime(2017, 3, 6)

###15 years of data
starts = datetime(2003, 1, 1)
endts = datetime(2017, 3, 5)

###15 years of data
starts = datetime(2017, 3, 3)
endts = datetime(2017, 11, 17)
#endts = datetime(2017, 3, 6)
