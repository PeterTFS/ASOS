...
# Updated: 02/12/2018 Add Solar Raditaion value according to SOW based on table provided by Mike
#-------------------------------------------------------------------------------
import time
import os
import sys
...
# table of Solar Radiation with SOW
# dict_SRvsSOW =
# {
#     0 : 750,
#     1 : 600,
...
def IntepreteASOS(csvfile,STATION,ID):
    ...
    with open(csvfile) as filt_csv:
        ...
        print "records for this station: ",len(asos),len(asos.index)
        if len(asos.index) > 48:
            ...
            #Here may need to do some work on when
        else:
            print 'There are total', len(asos.index),' Records for Station: ', STATION
        ...
        X9 = {'W98':'W98', 'Station Number':'000000', 'Ob Date':'YYYYMMDD', 'Ob Time':0,
                  ...
                  'MoistType':2, 'MeasType':1, 'SeasonCode':-999, 'SolarRad':0
                }
...
for STATION,ID in Stations.items():
    try:
        ...
        URL='https://www.aviationweather.gov/adds/dataserver_current/httpparam?dataSource=metars&requestType=retrieve&format=csv&startTime=%s&endTime=%s&stationString=%s'%(StartTime,EndTime,STATION,)
        print URL
