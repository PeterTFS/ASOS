...
##Tried Updated on 03/16/2016 for an automatic process
seasons = [(1, (date(Y,  1,  1),  date(Y,  3, 15))),
           (2, (date(Y,  3, 16),  date(Y,  6, 21))),
           (3, (date(Y,  6, 22),  date(Y,  11, 22))),
           (4, (date(Y,  11, 23),  date(Y, 12, 20))),
           (1, (date(Y, 12, 21),  date(Y, 12, 31)))]
...
##                    F9.write( FormatFW9( X9 ) +'\n' )
                    ...
                    #Update on 03/22/2016 Only write the obsType is 'O'
                    if X13['Type'] == 'O':
...
starts = datetime(2016, 3, 16)
endts = datetime(2016, 4, 5)
