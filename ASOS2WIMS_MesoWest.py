...
def StateOfWeather(X, row):
    ...
    if CurrentHourPrecip <= 0.1:
        if not wxstring.find('Clear') == -1:
            ...
            X['State of Weather'] = 2
        elif not raw_input.find('SCT','FEW') == -1:
            X['State of Weather'] = 1
        elif not raw_input.find('CLR') == -1:
            X['State of Weather'] = 0

    else:# only to see if there are rain
        if not wxstring.find('Thunderstorm') == -1:  # changed from DSNT to LTG 09182015
...
print herbaceousGreennessF[417601]
...
def IntepreteASOSWest(csvfile, STATION, ID):
    ...
    asos[['weather_condition_set_1d', 'metar_set_1']] = asos[['weather_condition_set_1d', 'metar_set_1']].astype(str)
    ...
    ASOS.loc[:,'Precipitation']=ASOS.loc[:,'Precipitation'].apply(CorrectPrcpAmount)
    #To define the Precipitation Duration hours
    ASOS.loc[:,'precip_duration']=ASOS.loc[:,'Precipitation'].apply(precipDuration)
...
for STATION, ID in Stations.items():
    try:
        ...
        #Precipitation query
        # precip = m.precip(stid='kfnl', start='201504261800',
        #                   end='201504271200', units='precip|in')
