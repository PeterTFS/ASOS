...
"""
Based on the discussion with Mike on 02/26/2018 which suggesting only 5 number will be assigned 
3 represents all conditions with 5 and above
Weather Code (MesoWest: 17 Unique)		Note
Mostly Clear	0	
Clear	0	
Mostly Cloudy	2	
Partly Cloudy	1	
...
Light Rain	3	
Rain	3	
Light Rain/Thunderstorm	3	
Fog	4	"only when the RH >95% and with 'FOG' code
"
Heavy Rain	3	
Heavy Rain/Thunderstorm	3	
Thunderstorm	3	
Haze	1	Look at the cloud cover(FEW, OVC...)
Light Snow	3	
Snow	3	
Blowing Dust	1	Look at the cloud cover(FEW, OVC...)
...
def IntepreteASOSWest(asos, STATION, ID):
    ...
    #print asos
    #Here may be a place to do some tricks with the windspeed
    asos.loc[:, 'processed_wind_speed'] = asos.loc[:, 'wind_speed_set_1']

    #create a column just based on the date and hour for group function
    df.loc[:, 'Date_Hour'] = df.loc[:, ('date_time')].str[0:13]

    df.loc[:, 'processed_wind_speed'] = df.loc[:, 'wind_speed_set_1'].groupby(df['Date_Hour']).transform(Last10Min)
    df.loc[:, 'processed_wind_direction'] = df.loc[:, 'wind_direction_set_1'].groupby(df['Date_Hour']).transform(Last10Min)
